...
export default function Active() {
  ...
  const setCharacteristic = (name: typeof CHARACTERISTICS[number], value: number) => {
    save({ ...c, characteristics: { ...c.characteristics, [name]: value } })
  }

  const setSkill = (name: typeof SKILLS[number], value: number) => {
    save({ ...c, skills: { ...c.skills, [name]: Math.max(0, Math.min(5, value)) } })
  }

  const addEquipment = (name: string) => {
    const item = EQUIPMENT.find((equipment) => equipment.name === name)
    if (!item || item.cost === null) return
    save({ ...c, equipment: [...c.equipment, { id: crypto.randomUUID(), name: item.name, quantity: 1, cost: item.cost }] })
  }
  ...
  const renderEquipmentDetails = (details: string) => {
    const parts = details.split(';').map((part) => part.trim()).filter(Boolean)
    const combat = parts.find((part) => /Weapon,|damage|\bdamage\b/i.test(part))
    ...
    const special = parts.filter((part) => ![combat, range, magazine, ammo].includes(part))
    return (
      <div className="equipmentDetails">
        {range && <div className="equipmentMeta">📐 {range.replace(/^\s*/, '')}</div>}
        {combat && <div className="equipmentRule"><strong>{combat}</strong></div>}
        {special.map((part) => <div className="equipmentRule" key={part}>{part}</div>)}
  ...
  return (
    <Layout>
      <div className="activeTop">
        <div><div className="eyebrow">ACTIVE PERSONNEL</div><h1>{c.name || 'UNNAMED'}</h1></div>
      ...
      <div className="hud">
        ...
        <div><span>IP</span><b>{c.improvementPoints}</b></div>
        <div><span>CREDITS</span><b>{c.credits.toLocaleString()} Cr</b></div>
      </div>

      <div className="tabs">
        {(['overview', 'skills', 'inventory'] as const).map((tabName) => <button key={tabName} className={tab === tabName ? 'activeTab' : ''} onClick={() => setTab(tabName)}>{tabName.toUpperCase()}</button>)}
      </div>
      ...
      {tab === 'overview' && <div className="grid three">
        ...
        <section className="panel"><h2>RESOURCES</h2>
          <label>CREDITS<input type="number" min={0} value={c.credits} onChange={(e) => save({ ...c, credits: Math.max(0, Number(e.target.value) || 0) })} /></label>
          <label>IMPROVEMENT POINTS<input type="number" min={0} value={c.improvementPoints} onChange={(e) => save({ ...c, improvementPoints: Math.max(0, Number(e.target.value) || 0) })} /></label>
          <label>STAMINA<input type="number" min={0} max={c.stamina} value={c.currentStamina} onChange={(e) => setStam(Number(e.target.value) || 0)} /></label>
          <label>LUCK<input type="number" min={0} max={6} value={c.luck} onChange={(e) => setLuck(Number(e.target.value) || 0)} /></label>
        </section>
      ...
      {tab === 'inventory' && <section className="panel"><div className="inventory-head"><h2>INVENTORY</h2><select defaultValue="" onChange={(e) => { if (!e.target.value) return; addEquipment(e.target.value); e.currentTarget.value = '' }}><option value="">ADD EQUIPMENT...</option>{EQUIPMENT.map((item) => <option key={item.name} value={item.name} disabled={item.cost === null}>{item.name}</option>)}</select></div>
        <div className="equipmentGrid">
          {c.equipment.length === 0 ? <p className="muted">No equipment recorded.</p> : c.equipment.map((inventoryItem, index) => {
            ...
            return <article className="equipmentCard" key={inventoryItem.id}>
              <div className="equipmentCardHeader"><h3>{inventoryItem.name}</h3><button className="danger" onClick={() => removeEquipment(index)}>REMOVE</button></div>
              <div className="equipmentDivider" />
              <div className="equipmentFields">
                <div><strong>Properties:</strong> {details.properties || '—'}</div>
                <div><strong>Cost:</strong> {details.cost === null ? '—' : `${details.cost}Cr`}</div>
                <div><strong>Rarity:</strong> {details.rarity === null ? '—' : details.rarity}</div>
                <div><strong>Carry Capacity:</strong> {details.carry}</div>
              </div>
              {renderEquipmentDetails(details.details)}
